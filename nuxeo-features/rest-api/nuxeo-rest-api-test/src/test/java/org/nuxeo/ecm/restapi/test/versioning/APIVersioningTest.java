...
public class APIVersioningTest {
    ...
    public void testInvalidAPIVersion() throws IOException {
        ...
        response = getRESTAPIResource(null).path("foo").path("path1").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(404, r.getStatus());
            assertInvalidAPIVersionException(r.getEntityInputStream(), null);
        }
    ...
    protected void assertInvalidAPIVersionException(InputStream is, Object apiVersion) throws IOException {
        ...
        String expectedMessage = String.format("%s is not part of the valid versions: %s", apiVersion,
                APIVersion.VALID_VERSIONS);
        assertEquals(expectedMessage, jsonNode.get("message").asText());
    }

    @Test
    public void testUpdatedEndpoint() {
        ClientResponse response = getRESTAPIResource(1).path("foo").path("path1").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
        ...
        response = getRESTAPIResource(2).path("foo").path("path1").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(200, r.getStatus());
            String body = r.getEntity(String.class);
    ...
    public void testNewEndpoint() {
        ClientResponse response = getRESTAPIResource(1).path("foo").path("path2").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(404, r.getStatus());
        }

        response = getRESTAPIResource(2).path("foo").path("path2").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(200, r.getStatus());
            String body = r.getEntity(String.class);
            assertEquals("bar", body);
    ...
    public void testUpdatedWriter() throws Exception {
        ...
        response = getRESTAPIResource(2).path("foo").path("dummy").get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(200, r.getStatus());
            JsonNode jsonNode = MAPPER.readTree(r.getEntityInputStream());
            assertEquals("dummy", jsonNode.get("entity-type").asText());
    ...
    public void testUpdatedReader() {
        ...
        response = getRESTAPIResource(2).path("foo").path("dummy").post(ClientResponse.class, json);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(200, r.getStatus());
    ...
    public void testUpdatedEnricher() throws Exception {
        ...
        response = getRESTAPIResource(2).path("foo")
                                        .path("dummy")
                                        .header("enrichers.dummy", "dummy")
                                        .get(ClientResponse.class);
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
    ...
    public void testNewEnricher() throws Exception {
        ...
        try (CloseableClientResponse r = CloseableClientResponse.of(response)) {
            assertEquals(200, r.getStatus());
            JsonNode jsonNode = MAPPER.readTree(r.getEntityInputStream());
            assertNull(jsonNode.get("contextParameters").get("dummy2"));
        }
